...
class SpecialNews extends Model
{
    ...
    static function getSpecialNews($skip,$limit,$flag,$sSearch,$betweenOpt){
        if (empty($limit)){
            return [];
        }
        if (!isset($skip)){
        ...
        $query = SpecialNews::whereHas('user', function ($query) use($sSearch,$betweenOpt){
            if ($sSearch){
...
//        $query = SpecialNews::has('user');
    ...
    static function getSpecialNewsCount(){

        $count = SpecialNews::count();
    ...
    static function getHasNews($skip,$limit,$flag,$orOpt=array(),$betweenOpt=array(),$sid){
        ...
        $query = SpecialNews::whereHas('news', function ($query) use($sid) {
            $query->where('sid', $sid);
        });
        if (!empty($betweenOpt)){
            foreach ($betweenOpt as $k => $v){
    ...
    static function getHasNewsCount($sid){
        ...
        $count = SpecialNews::whereHas('news', function ($query) use($sid) {
            $query->where('sid', $sid);
        })->count();
