    ...
    <div id='PlatformDetail'>
        ...
        <Row type="flex" justify="center">
            <Col :xs="{'span':18}" :sm="{'span':18}" :lg="{'span':18}" :class-name="'container'">
                ...
                <!--<div class="bread">-->
                    <!--<div class="bread-text"><a href="/">首页</a> > <a>交易</a> > <a>代理招商</a> > 代理招商详情页</div>-->
                <!--</div>-->
                ...
                <Breadcrumb separator=">" style="margin: 0 0 15px 0;">
                    ...
                    <BreadcrumbItem>代理招商详情</BreadcrumbItem>
                </Breadcrumb>
                ...
                <Row type="flex" justify="space-between">
                    <Col :xs="{'span':16}" :sm="{'span':16}" :lg="{'span':16}" :class-name="'page-left'">
                        <Row :class-name="'bns-detail'">
                            <Col :xs="{'span':24}" :sm="{'span':24}" :lg="{'span':24}">
                                <h2 class="bns-title">{{platform_info.title}}</h2>
                                <p class="bns-author">
                                    <span class="bns-name title-hidden" @click="go('/user/userzone/'+platform_info.user.id)">{{platform_info.user.username}}</span>
                                    <span class="bns-pubdate">{{platform_info.updated_at}}</span>
                                </p>
                                <div class="bns-type">
                                    <ul class="marks">
                                        <li><a>业务类型</a></li>
                                        <li v-for="(game,key) in platform_info.gameList"><a :href="'/trade/platform/'+key+'/'" target="_blank">{{game}}</a></li>
                                    </ul>
                                    <ul class="links">
                                        <li>平台名称：<a :href="platform_info.url" target="_blank">{{platform_info.enterprise}}</a></li>
                                        <li>佣金结算：<a :href="'/trade/platform/0/'+platform_info.settlement" target="_blank">{{platform_info.settlementName}}</a></li>
                                    </ul>
                                </div>
                                <div class="bns-content">
                                    ...
                                    <div class="bns-article" v-html="platform_info.content">
                                    </div>
                                </div>
                                <p class="bns-notice">
                                    ...
                                    <TabPane label="平台招募" name="proxy">
                                        <div class="trade-panel">
                                            <Col :xs="{'span':24}" :sm="{'span':24}" :lg="{'span':24}" v-for="(proxy,key) in proxyList" :key="key" :class-name="'list-frame'">
                                                ...
                                                <div class='list-body'>
                                                    <span class="brief">{{proxy.brief?proxy.brief.substring(0,100):''}}
                                                        <span v-if="proxy.brief&&proxy.brief.length>100">...</span>
                                        ...
                                        <div v-if="!loading">
                                            <Col :xs="{'span':8,'offset':8}" :sm="{'span':8,'offset':8}" :lg="{'span':8,'offset':8}">
                                                <center>
                                                    <Button class="load-more" @click="get_more"  v-if="more_proxy">        浏览更多
                                                    </Button>
                                                </center>
                                            </Col>
                                        </div>
                                    ...
                                    <TabPane label="代理寻商" name="trade">
                                        <div class="proxy-panel">
                                            <Col :xs="{'span':24}" :sm="{'span':24}" :lg="{'span':24}" v-for="(trade,key) in tradeList" :key="key" :class-name="'list-frame'">
                                                ...
                                                <div class='list-body'>
                                                    <span class="brief">{{trade.brief?trade.brief.substring(0,100):''}}
                                                        <span v-if="trade.brief&&trade.brief.length>100">...</span>
                    ...
                    <Col :xs="{'span':7}" :sm="{'span':7}" :lg="{'span':7}" :class-name="'page-right'" v-if="init">
                        <Row type="flex" justify="center">
                            <Col :xs="{'span':24}" :sm="{'span':24}" :lg="{'span':24}">
                                <div class="attention-frame" v-if="platform_info.confirm && platform_info.type == 1">
                                    <Row type="flex" justify="space-between">
                                        ...
                                        <Col :xs="{'span':24}" :sm="{'span':24}" :lg="{'span':24}">
                                            <ul class="bns-certification">
                                                <li v-for="(plate,key) in plateList" v-if="platform_info.plate & key">
                                                    <i class="ctfimg" :class="plate.class" :alt="plate.title"></i>
                                                    <div class="ctfintro">
                                                        <h2 :style="'color: '+plate.color+';'">{{plate.title}}认证 <a :href="'/front/platform_details/confirm/'+platform_info.id+'/'+key" target="_blank">查看认证证书</a></h2>
                                                        <p>
                                ...
                                <div class='bns-contact' v-if="platform_info.qq || platform_info.skype || platform_info.phone || platform_info.wechat || platform_info.email">
                                    <h2 class="cntTitle">联系方式</h2>
                                    <ul>
                                        <li v-if="platform_info.qq">
                                            <template v-if="isLogin">
                                                <i class="sns-img qq1"></i>
                                                <h2>{{platform_info.qq}}</h2>
                                                <p>
                                                    <a class="text-copy" :data-clipboard-text="platform_info.qq">快速复制</a>
                                        ...
                                        <li v-if="platform_info.skype">
                                            <template v-if="isLogin">
                                                <i class="sns-img skype2"></i>
                                                <h2>{{platform_info.skype}}</h2>
                                                <p>
                                                    <a class="text-copy" :data-clipboard-text="platform_info.skype">快速复制</a>
                                                    <!-- <a href="http://wpa.b.qq.com/cgi/wpa.php?ln=1&key=XzgwMDE4Njg5NF80NTE0NDNfODAwMTg2ODk0XzJf" target="_blank">在线联系</a> -->
                                                </p>
                                            </template>
                                            <template v-else>
                                        ...
                                        <li v-if="platform_info.phone">
                                            <template v-if="isLogin">
                                                <i class="sns-img phone3"></i>
                                                <h2>{{platform_info.phone}}</h2>
                                                <p>
                                                    <a class="text-copy" :data-clipboard-text="platform_info.phone">快速复制</a>
                                                </p>
                                            </template>
                                            <template v-else>
                                                <i class="sns-img phone3"></i>
                                        ...
                                        <li v-if="platform_info.wechat">
                                            <template v-if="isLogin">
                                                <i class="sns-img weixin4"></i>
                                                <h2>{{platform_info.wechat}}</h2>
                                                <p>
                                                    <a class="text-copy" :data-clipboard-text="platform_info.wechat">快速复制</a>
                                                </p>
                                            </template>
                                            <template v-else>
                                                <i class="sns-img weixin4"></i>
                                        ...
                                        <li v-if="platform_info.email">
                                            <template v-if="isLogin">
                                                <i class="sns-img email5"></i>
                                                <h2>{{platform_info.email}}</h2>
                                                <p>
                                                    <a class="text-copy" :data-clipboard-text="platform_info.email">快速复制</a>
                                                </p>
                                            </template>
                                            <template v-else>
                                                <i class="sns-img email5"></i>
...
export default{
    ...
    mounted(){
        document.title = '菠菜圈| 代理招商详情';
        var v = this;
        this.platform_id = this.$route.params.platform_id;
        this.https.get('/front/platform_details/render', {
            params: {
                bid: v.platform_id
            }
        }).then((r)=>{
            if (r.data.platform_info instanceof Array && r.data.platform_info.length == 0){
                v.$Message.warning('该信息不存在或已删除!');
                setTimeout(function () {
                    v.$router.push('/trade/platform');
                },2000)
            }else{                
                v.platform_info = r.data.platform_info;
                try {
                    v.imgList = $.parseJSON(v.platform_info.pic);
                } catch(err) {
                    v.imgList = [];
                }
