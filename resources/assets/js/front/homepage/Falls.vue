		...
		<ul>
			<li v-for="(item,key) in falls">
				<div v-if="item.model=='News'" class="news list">
					<div  class="img-zone">
						<div class="img-frame">
							<a class="title" v-bind:href="'/news/newspage/'+item.id" target="_blank">
					...
					<div class="content-zone">
						<div class="content-fame">
							<div class="line2 pointer" style="height:50px;"><a class="title" v-bind:href="'/news/newspage/'+item.id" target="_blank">{{item.title}}</a></div>
							<div class="content-info">
								<div class="right-info">
									...
									&nbsp&nbsp|&nbsp&nbsp<span class="shoucang-frame" @click="collection(item.id,item.is_collection,key)"><i class="icon icon-shoucang" v-bind:class="{'collected':item.is_collection}"></i>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp{{item.collection_count}}</span>		
								</div>
								<div class="byself" v-if="item.byself">原创</div>
								<div class="tags pointer" @click="changeCat({type:'News',id:item.cid})">{{item.category_name}}</div>&nbsp&nbsp
				...
				<div v-if="item.model=='Report'" class="report list">
					<div class="content-fame">
						<div class="line1 pointer"><a class="title" v-bind:href="'/news/reportDetail/'+item.id" target="_blank">{{item.title}}</a></div>
						<div class="content-info">
							<div class="right-info">
								<i class="icon icon-eye1"></i>&nbsp&nbsp{{item.f_view}}
							...
							<div class="tags pointer" @click="changeCat({type:'Report'})">官方报告</div>&nbsp&nbsp
							<a v-bind:href="'/user/userzone/'+item.uid" target="_blank">
								<div class="avatar">
									<div class="avatar-frame-new">									
										<img v-bind:src="item.user_image">									
				...
				<div v-if="item.model=='UserData'" class="userdata list">
					<div class="content-fame">
						<div class="title pointer"><a class="title" v-bind:href="'/userdata/UserDataDetail/'+item.id" target="_blank">{{item.title}}</a></div>
						<div class="content-info">
							<div class="right-info">
								<i class="icon icon-eye1"></i>&nbsp&nbsp{{item.f_view}}
							...
							<div class="tags pointer" @click="changeCat({type:'UserData'})">产业资源</div>&nbsp&nbsp
							<a v-bind:href="'/user/userzone/'+item.uid" target="_blank">
								<div class="avatar">
									<div class="avatar-frame-new">																		
										<img v-bind:src="item.user_image">
				...
				<div v-if="item.model=='Question'" class="question list">
					<div class="content-fame">
						<div class="line1 pointer"><a class="title" v-bind:href="'/questiondetail/'+item.id" target="_blank">{{item.title}}</a></div>
						<div class="content-info">
							<div class="right-info">
								<i class="icon icon-eye1"></i>&nbsp&nbsp{{item.f_view}}
							...
							<div class="tags pointer" @click="changeCat({type:'Question'})">互动问答</div>&nbsp&nbsp
							<a v-bind:href="'/user/userzone/'+item.uid" target="_blank">	
								<div class="avatar">
									<div class="avatar-frame-new">					
										<img v-bind:src="item.user_image">
				...
				<div v-if="item.model=='Trend'" class="trend list">
					<div class="content-fame">
						<div class="line1 pointer"><a class="title" v-bind:href="'/news/trend?id='+item.id" target="_blank">{{item.title}}</a></div>
						<div class="content-info">
							<div class="right-info">
								<i class="icon icon-eye1"></i>&nbsp&nbsp{{item.f_view}}
							</div>
							<div class="tags pointer" @click="changeCat({type:'Trend'})">产品动态</div>&nbsp&nbsp
							<a v-bind:href="'/user/userzone/'+item.uid" target="_blank">
								<div class="avatar">
									<div class="avatar-frame-new">
										<img v-bind:src="item.user_image">
				...
				<div v-if="item.model=='Business'" class="business list">
					<div class="content-fame">
						<div class="line1 pointer"><a class="title" v-bind:href="'/trade/BusinessDetail/'+item.id" target="_blank">{{item.title}}</a></div>
						<div class="content-info">
							<div class="right-info">
								<i class="icon icon-eye1"></i>&nbsp&nbsp{{item.f_view}}
							</div>							
							<div class="tags pointer" @click="changeCat({type:'Business'})">供求商讯</div>&nbsp&nbsp
							<a v-bind:href="'/user/userzone/'+item.uid" target="_blank">
								<div class="avatar">
									<div class="avatar-frame-new">
										<img v-bind:src="item.user_image">
				...
				<div v-if="item.model=='Platform'" class="platform list">
					...
					<div class="content-zone" v-bind:class="{'content-zone-width':!item.logo}">
						<div class="content-fame">
							<div class="line1 pointer"><a class="title" v-bind:href="'/trade/PlatformDetail/'+item.id" target="_blank">{{item.title}}</a></div>
							<div class="content-info">
								<div class="right-info">
									<i class="icon icon-eye1"></i>&nbsp&nbsp{{item.f_view}}	
								</div>
								<div class="tags pointer" @click="changeCat({type:'Platform'})">代理招商</div>&nbsp&nbsp
								<a v-bind:href="'/user/userzone/'+item.uid" target="_blank">
									<div class="avatar">
										<div class="avatar-frame-new">
											<img v-bind:src="item.user_image">
...
<!-- 								<img v-bind:src="item.image[0]" onerror="this.src='/static/noimg.png'"> -->
					...
					<div class="content-zone" v-bind:class="{'content-zone-width':item.image.length&&item.image.length>=4||item.image.length==0}">
						<div class="content-fame">
							...
							<div class="pointer"><a class="title" v-bind:href="'/news/micro?id='+item.id" target="_blank"><div class="line1 micro-content" v-html="item.content"></div></a></div>
							<a class="title" v-bind:href="'/news/micro?id='+item.id" target="_blank">
							...
							<div class="content-info">
								...
								<div class="tags pointer" @click="changeCat({type:'Micro'})">微动态</div>
								<div class="time">&nbsp&nbsp{{item.time}}</div>
							</div>
						</div>
					</div>					
	...
	<Col class="demo-spin-col" :xs="{'span':8,'offset':8}" :sm="{'span':8,'offset':8}" :lg="{'span':8,'offset':8}"  v-if="loading">
        <Spin fix>
	...
	export default{
		...
		mounted(){
			...
			　　var windowHeight = $(this).height();
				if((scrollHeight - windowHeight -1 - scrollTop < 0)&&!v.loading&&v.has_more){
					v.loading = true
			　　　　 v.getFalls()
			　　}
			})
