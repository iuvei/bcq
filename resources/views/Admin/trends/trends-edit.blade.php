...
<article class="page-container">
	<form class="form form-horizontal" id="form-admin">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>所属类型(type)：</label>
			<input type="hidden" name='type' id="type">
			<div class="formControls col-xs-8 col-sm-8 skin-minimal">
				@foreach(config('base.trendsType') as $key => $val)
					<div class="check-box">
						<input type="checkbox" id="ty-{{$key}}" value="{{$key}}" name="ty" @if($obj->type & $key) checked @endif>
						<label for="ty-{{$key}}">{{$val}}</label>
					</div>
				@endforeach
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>产品官网(product_url)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="{{$obj->product_url}}" placeholder="" id="product_url" name="product_url">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			@if(!$obj->status)
				<div class="col-xs-1 col-sm-1">
					<button type="button" class="btn btn-danger radius" id="admin-return" onclick="message_send('发送私信','/{{config('base.admin')}}/message/message_send_add/{{$obj->uid}}/{{$table}}/{{$obj->id}}/2/产品动态/{{$obj->created_at}}','500','250')"><i class="icon-ok"></i> 退回通知</button>
				</div>
			@endif
		</div>
	</form>
        ...
        $("#form-admin").validate({
            ...
            submitHandler:function(form){
                ...
                $(form).ajaxSubmit({
                    ...
                    url: '/{{config('base.admin')}}/trends/trends_edit/{{$obj->id}}', // 需要提交的 url
