	...
	<form class="form form-horizontal" id="form-admin">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8 skin-minimal">
				@foreach(config('base.settlement') as $key => $val)
					<div class="radio-box">
						<input type="radio" id="settlement-{{$key}}" value="{{$key}}" name="settlement" @if($obj->settlement == $key) checked @endif>
		...
		@if($obj->type == 1)
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8 skin-minimal">
					@foreach(config('base.scale') as $key => $val)
						<div class="radio-box">
							<input type="radio" id="scale-{{$key}}" value="{{$key}}" name="scale" @if($obj->hot == $key) checked @endif>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					<input type="text" class="input-text" value="{{$obj->rate}}" placeholder="" id="rate" name="rate">
				</div>
			</div>
		@endif
		<div class="row cl">
		...
		<div class="row cl">
			...
			@if(!$obj->status)
				<div class="col-xs-1 col-sm-1">
					<button type="button" class="btn btn-danger radius" id="admin-return" onclick="message_send('发送私信','/{{config('base.admin')}}/message/message_send_add/{{$obj->uid}}/{{$table}}/{{$obj->id}}/2/代理招商/{{$obj->created_at}}','500','250')"><i class="icon-ok"></i> 退回通知</button>
				</div>
			@endif
		</div>
	</form>
...
$("#form-admin").validate({
	...
	submitHandler:function(form){
		...
		$(form).ajaxSubmit({
			...
			url: '/{{config('base.admin')}}/platform/platform_edit/{{$obj->id}}', // 需要提交的 url
...
(function( $ ){
    ...
    $(function() {
        ...
        $.ajax({
            url: '/{{config('base.admin')}}/platform/ajax_get_file/{{$obj->id}}',
            type: 'GET',
