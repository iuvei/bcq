...
<div class="page-container">
    <form method="POST" id="list_form" action="/{{config('base.admin')}}/trends_comment/trends_comment_list">
        {{ csrf_field() }}
        <div class="text-c">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a href="javascript:;" onclick="datastatus(1,'/{{config('base.admin')}}/trends_comment/datastatus')" class="btn btn-success radius"><i class="Hui-iconfont">&#xe6e1;</i> 批量启用</a>
            <a href="javascript:;" onclick="datastatus(0,'/{{config('base.admin')}}/trends_comment/datastatus')" class="btn btn-warning radius"><i class="Hui-iconfont">&#xe631;</i> 批量禁用</a>
            <a href="javascript:;" onclick="datastatus(-1,'/{{config('base.admin')}}/trends_comment/datastatus')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
        </span>
...
<script type="text/javascript">
    $(function(){
        ...
        $('.table-sort').dataTable({
            ...
            "sAjaxSource": "/{{config('base.admin')}}/trends_comment/trends_comment_ajax_list",
            "aaSorting": [
                [0, "desc"],
            ],
            "aoColumns": [
                ...
                {"mData": "tid" , "mRender": function ( data, type, full )
                {
                    var title = data;
                    if(full.trends){
                        title = full.trends.title;
                    }
                    return "<div class='label radius' title='"+title+"'>"+data+"</div>";
                }
                },
                {"mData": "tcid" , "mRender": function ( data, type, full )
                {
                    var content = data;
                    if(full.comment){
                ...
                {"mData": "id","mRender": function ( data, type, full )
                {
                    return "<a title='编辑' href='javascript:;' onclick=\"trends_comment_edit('编辑','/{{config('base.admin')}}/trends_comment/trends_comment_edit/"+data+"','1200','')\" class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
                }
                }
            ],
            "oLanguage": {
    ...
    /*产品评论-编辑*/
    function trends_comment_edit(title,url,w,h){
