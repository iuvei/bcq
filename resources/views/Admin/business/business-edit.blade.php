...
<article class="page-container">
	<form class="form form-horizontal" id="form-admin">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>服务类型(sbid)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<span class="select-box">
					<select id="sbid" name="sbid" class="select">
						<option value="">未选择</option>
						@foreach($category as $key => $val)
							@if($key == 0)
								@foreach($val as $k => $v)
									<option value="{{$v->id}}" @if($v->id == $obj->bcategory->fid) selected @endif>{{$v->cname}}</option>
								@endforeach
							@endif
						@endforeach
					</select>
		...
		<div id="bidSelect" @if($obj->bid) class="row cl" @endif>
			@if($obj->bid)
				<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>服务类型(bid)：</label>
					<div class="formControls col-xs-8 col-sm-8">
					<span class="select-box">
						<select id="bid" name="bid" class="select">
							<option value="">未选择</option>
							@foreach($category as $key => $val)
								@if($key == $obj->bcategory->fid)
									@foreach($val as $k => $v)
										<option value="{{$v->id}}" @if($v->id == $obj->bid) selected @endif>{{$v->cname}}</option>
									@endforeach
								@endif
							@endforeach
						</select>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div class="uploader-list-container">
					<input type="hidden" name="pic" id="pic" value="{{trim(trim($obj->pic,'['),']')}}"/>
					<div class="queueList">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="{{$obj->brief}}" placeholder="" id="brief" name="brief">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="{{$obj->contactperson}}" placeholder="" id="contactperson" name="contactperson">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">电话(phone)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="{{$obj->phone}}" placeholder="" id="phone" name="phone">
			</div>
		</div>
		<div class="row cl">
		...
		@if($obj->type == 1)
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8 skin-minimal">
					<div class="radio-box">
						<input type="radio" id="confirm-1" value="0" name="confirm" @if($obj->hot == 0) checked @endif>
						<label for="confirm-1">否</label>
					</div>
					<div class="radio-box">
						<input type="radio" id="confirm-2" value="1" name="confirm" @if($obj->hot == 1) checked @endif>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					<input type="text" onfocus="WdatePicker({dateFmt: 'yyyy-MM-dd 00:00:00'})" id="validitytime" name="validitytime" class="input-text Wdate" style="width:40%;" value="{{$obj->validitytime}}">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">保证金(money)：</label>
				<div class="formControls col-xs-8 col-sm-8">
					<input type="text" class="input-text" value="{{$obj->money}}" placeholder="" id="money" name="money">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">平台名称(enterprise)：</label>
				<div class="formControls col-xs-8 col-sm-8">
					<input type="text" class="input-text" value="{{$obj->enterprise}}" placeholder="" id="enterprise" name="enterprise">
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					<div class="uploader-list-container">
						...
						<input type="hidden" name="logo" id="logo" value="{{$obj->image}}" />
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8 skin-minimal">
					@foreach(config('base.year') as $key => $val)
						<div class="radio-box">
							<input type="radio" id="year-{{$key}}" value="{{$key}}" name="year" @if($obj->year == $key) checked @endif>
							<label for="year-{{$key}}">{{$val}}</label>
						</div>
					@endforeach
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8 skin-minimal">
					@foreach(config('base.scale') as $key => $val)
						<div class="radio-box">
							<input type="radio" id="scale-{{$key}}" value="{{$key}}" name="scale" @if($obj->scale == $key) checked @endif>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					<input type="text" class="input-text" value="{{$obj->personnel}}" placeholder="" id="personnel" name="personnel">
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8 skin-minimal">
					@foreach(config('base.plate') as $key => $val)
						<div class="check-box">
							<input type="checkbox" id="pla-{{$key}}" value="{{$key}}" name="pla" @if($obj->plate & $key) checked @endif>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8 skin-minimal">
				<div class="radio-box">
					<input type="radio" id="hot-1" value="0" name="hot" @if($obj->hot == 0) checked @endif>
					<label for="hot-1">否</label>
				</div>
				<div class="radio-box">
					<input type="radio" id="hot-2" value="1" name="hot" @if($obj->hot == 1) checked @endif>
		...
		<div class="row cl">
			...
			@if(!$obj->status)
				<div class="col-xs-1 col-sm-1">
					<button type="button" class="btn btn-danger radius" id="admin-return" onclick="message_send('发送私信','/{{config('base.admin')}}/message/message_send_add/{{$obj->uid}}/{{$table}}/{{$obj->id}}/2/问答/{{$obj->created_at}}','500','250')"><i class="icon-ok"></i> 退回通知</button>
				</div>
			@endif
		</div>
	</form>
...
<script type="text/javascript">
    ...
    $('.queueList').on('click','.filelist li .imgWrap',function(){
        layer.photos({
			photos: $(this),
...
$('#sbid').on('change',function(){
	var sbid = parseInt($('#sbid').val());
	var strArr = [];
	if(sbid){
		$('#bidSelect').addClass('row cl');
		strArr.push('<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>子类型(bid)：</label>');
		strArr.push('<div class="formControls col-xs-8 col-sm-8">');
		strArr.push('<span class="select-box">');
		strArr.push('<select id="bid" name="bid" class="select">');
		strArr.push('<option value="">未选择</option>');
		$.each(category[sbid],function (key,val) {
		    if(obj.bid == val.id){
                strArr.push('<option value="'+val.id+'" selected>'+val.cname+'</option>');
            }else{
                strArr.push('<option value="'+val.id+'">'+val.cname+'</option>');
            }
		});
		strArr.push('</select>');
		strArr.push('</span>');
		strArr.push('</div>');
		$('#bidSelect').html(strArr.join("\r\n"));
	}else{
		$('#bidSelect').removeClass('row cl');
		$('#bidSelect').html(strArr.join(''));
	}
});
...
$("#form-admin").validate({
	...
	submitHandler:function(form){
		...
		$(form).ajaxSubmit({
			...
			url: '/{{config('base.admin')}}/business/business_edit/{{$obj->id}}', // 需要提交的 url
...
(function( $ ){
    ...
    $(function() {
        ...
        $.ajax({
            url: '/{{config('base.admin')}}/business/ajax_get_file/{{$obj->id}}',
            type: 'GET',
...
//        });
         ...
         uploader.on('filesQueued', function(file) {
             if(file[0]){
                 $("#"+file[0].source.id).attr('imgsrc',file[0].source.src).find('.imgWrap').html("<img src="+file[0].source.src+" style='width: 100%;height: 100%;' />")
             }
