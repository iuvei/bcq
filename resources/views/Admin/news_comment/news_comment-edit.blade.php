		...
		<div class="row cl">
			...
			@if(!$obj->status)
				<div class="col-xs-1 col-sm-1">
					<button type="button" class="btn btn-danger radius" id="admin-return" onclick="message_send('发送私信','/{{config('base.admin')}}/message/message_send_add/{{$obj->uid}}/{{$table}}/{{$obj->id}}/2/产业资讯评论/{{$obj->created_at}}','500','250')"><i class="icon-ok"></i> 退回通知</button>
				</div>
			@endif
		</div>
	</form>
        ...
        $("#form-admin").validate({
            ...
            submitHandler:function(form){
                ...
                $(form).ajaxSubmit({
                    ...
                    url: '/{{config('base.admin')}}/news_comment/news_comment_edit/{{$obj->id}}', // 需要提交的 url
