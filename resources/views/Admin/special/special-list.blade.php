...
<div class="page-container">
    <form method="POST" id="list_form" action="/{{config('base.admin')}}/special/special_list">
        {{ csrf_field() }}
        <div class="text-c">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a href="javascript:;" onclick="datastatus(1,'/{{config('base.admin')}}/special/datastatus')" class="btn btn-success radius"><i class="Hui-iconfont">&#xe6e1;</i> 批量启用</a>
            <a href="javascript:;" onclick="datastatus(0,'/{{config('base.admin')}}/special/datastatus')" class="btn btn-warning radius"><i class="Hui-iconfont">&#xe631;</i> 批量禁用</a>
            <a href="javascript:;" onclick="datastatus(-1,'/{{config('base.admin')}}/special/datastatus')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
            <a class="btn btn-primary radius" onclick="special_add('添加专题','/{{config('base.admin')}}/special/special_add','1200','')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加专题</a>
        </span>
...
<script type="text/javascript">
    $(function(){
        ...
        $('.table-sort').dataTable({
            ...
            "sAjaxSource": "/{{config('base.admin')}}/special/special_ajax_list",
            "aaSorting": [
                [0, "desc"],
            ],
            "aoColumns": [
                ...
                {"mData": "id","mRender": function ( data, type, full )
                {
                    return "<a title='编辑' href='javascript:;' onclick=\"special_edit('编辑','/{{config('base.admin')}}/special/special_edit/"+data+"','1200','')\" class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a> &emsp; <a title='专题资讯列表' href='javascript:;' onclick=\"special_news_list('专题资讯列表','/{{config('base.admin')}}/special_news/special_news_list/"+data+"','1200','')\" class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe645;</i></a>";
                }
                }
            ],
            "oLanguage": {
    ...
    /*专题-添加*/
    function special_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
    /*专题-编辑*/
    function special_edit(title,url,w,h){
        layer_show(title,url,w,h);
    }
    /*专题-添加资讯*/
    function special_news_list(title,url,w,h){
        layer_show(title,url,w,h);
    }
</script>
</body>
