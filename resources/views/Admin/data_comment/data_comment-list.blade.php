...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产业资源下载管理 <span class="c-gray en">&gt;</span> 产业资源下载评论列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <form method="POST" id="list_form" action="/{{config('base.admin')}}/data_comment/data_comment_list">
        {{ csrf_field() }}
        <div class="text-c">
            ...
            <button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜产业资源下载评论</button>
        </div>
    </form>
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a href="javascript:;" onclick="datastatus(1,'/{{config('base.admin')}}/data_comment/datastatus')" class="btn btn-success radius"><i class="Hui-iconfont">&#xe6e1;</i> 批量启用</a>
            <a href="javascript:;" onclick="datastatus(0,'/{{config('base.admin')}}/data_comment/datastatus')" class="btn btn-warning radius"><i class="Hui-iconfont">&#xe631;</i> 批量禁用</a>
            <a href="javascript:;" onclick="datastatus(-1,'/{{config('base.admin')}}/data_comment/datastatus')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
        </span>
...
<script type="text/javascript">
    $(function(){
        ...
        $('.table-sort').dataTable({
            ...
            "sAjaxSource": "/{{config('base.admin')}}/data_comment/data_comment_ajax_list",
            "aaSorting": [
                [0, "desc"],
            ],
            "aoColumns": [
                ...
                {"mData": "did" , "mRender": function ( data, type, full )
                {
                    var title = data;
                    if(full.data){
                ...
                {"mData": "dcid" , "mRender": function ( data, type, full )
                {
                    if (data){
                        var content = data;
                        if(full.comment){
                ...
                {"mData": "id","mRender": function ( data, type, full )
                {
                    return "<a title='编辑' href='javascript:;' onclick=\"data_comment_edit('编辑','/{{config('base.admin')}}/data_comment/data_comment_edit/"+data+"','1200','')\" class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
                }
                }
            ],
            "oLanguage": {
    ...
    /*产业资源下载评论-编辑*/
    function data_comment_edit(title,url,w,h){
