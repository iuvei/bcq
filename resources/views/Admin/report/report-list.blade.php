...
<div class="page-container">
    <form method="POST" id="list_form" action="/{{config('base.admin')}}/report/report_list">
        {{ csrf_field() }}
        <div class="text-c">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a href="javascript:;" onclick="datastatus(1,'/{{config('base.admin')}}/report/datastatus')" class="btn btn-success radius"><i class="Hui-iconfont">&#xe6e1;</i> 批量启用</a>
            <a href="javascript:;" onclick="datastatus(0,'/{{config('base.admin')}}/report/datastatus')" class="btn btn-warning radius"><i class="Hui-iconfont">&#xe631;</i> 批量禁用</a>
            <a href="javascript:;" onclick="datastatus(-1,'/{{config('base.admin')}}/report/datastatus')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
            <a class="btn btn-primary radius" onclick="report_add('添加资料','/{{config('base.admin')}}/report/report_add','1200','')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加资料</a>
        </span>
...
<script type="text/javascript">
    $(function(){
        ...
        $('.table-sort').dataTable({
            ...
            "sAjaxSource": "/{{config('base.admin')}}/report/report_ajax_list",
            "aaSorting": [
                [0, "desc"],
            ],
            "aoColumns": [
                ...
                {"mData": "id","mRender": function ( data, type, full )
                {
                    return "<a title='编辑' href='javascript:;' onclick=\"report_edit('编辑','/{{config('base.admin')}}/report/report_edit/"+data+"','1200','')\" class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
                }
                }
            ],
            "oLanguage": {
    ...
    /*资料-添加*/
    function report_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
    /*资料-编辑*/
    function report_edit(title,url,w,h){
        layer_show(title,url,w,h);
