...
<article class="page-container">
	<form class="form form-horizontal" id="form-admin">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>动态内容(content)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<textarea id="content" name="content" type="text/plain">{{$obj->content}}</textarea>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">动态图片(image)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<div class="uploader-list-container">
					<input type="hidden" name="image" id="image" value="{{trim(trim($obj->image,'['),']')}}"/>
					<div class="queueList">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>用户类型(utype)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<span class="select-box">
					<select name="utype" class="select">
						<option value="0" @if(intval($obj->utype) == 0) selected @endif>普通用户</option>
						<option value="1" @if(intval($obj->utype) == 1) selected @endif>游戏用户</option>
					</select>
				</span>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="{{$obj->top}}" placeholder="" id="view" name="view">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			<div class="col-xs-1 col-sm-1 col-xs-offset-2 col-sm-offset-2">
				...
				<div class="col-xs-1 col-sm-1">
					<button type="button" class="btn btn-danger radius" id="admin-return" onclick="message_send('发送私信','/{{config('base.admin')}}/message/message_send_add/{{$obj->uid}}/{{$table}}/{{$obj->id}}/2/微动态/{{$obj->created_at}}','500','250')"><i class="icon-ok"></i> 退回通知</button>
				</div>
		</div>
	</form>
</article>
            ...
            submitHandler:function(form){
                ...
                $('input[name=ty]').each(function() {
                    if ($(this).parent().hasClass('checked')) {
                        type_total = Number($(this).val())+type_total;
                    }
                });
                $("#type").val(type_total);
                ...
                $(form).ajaxSubmit({
                    ...
                    url: '/{{config('base.admin')}}/micro/micro_edit/{{$obj->id}}', // 需要提交的 url
    ...
    (function( $ ){
        ...
        $(function() {
            ...
            uploader = WebUploader.create({
                ...
                fileNumLimit: 9,
                fileSizeLimit: 9 * 2 * 1024 * 1024,    // 5 M
                fileSingleSizeLimit: 10 * 1024 * 1024    // 50 M
            });

            $.ajax({
                url: '/{{config('base.admin')}}/micro/ajax_get_file/{{$obj->id}}',
                type: 'GET',
