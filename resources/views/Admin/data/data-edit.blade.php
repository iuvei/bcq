...
<article class="page-container">
	<form class="form form-horizontal" id="form-admin">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>简介(brief)：</label>
			<div class="formControls col-xs-8 col-sm-8">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>资料类型(type)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<span class="select-box">
					<select name="type" class="select">
						<option value="">未选择</option>
						@foreach(config('base.dataType') as $key => $val)
							<option value="{{$key}}" @if($obj->type == $key) selected @endif>{{$val}}</option>
						@endforeach
					</select>
				</span>
			</div>
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>资料：</label>
			<div class="formControls col-xs-8 col-sm-8"> <span class="btn-upload form-group">
				<input class="input-text upload-url" type="text" name="uploadfile" id="uploadfile" readonly nullmsg="请添加附件！" style="width:200px" value="{{basename($obj->files->path)}}>
				...
				<input type="hidden" name="suffix" value="{{$obj->suffix}}">
				<input type="hidden" name="file" value="{{$obj->file}}">
				</span>
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			@if(!$obj->status)
				<div class="col-xs-1 col-sm-1">
					<button type="button" class="btn btn-danger radius" id="admin-return" onclick="message_send('发送私信','/{{config('base.admin')}}/message/message_send_add/{{$obj->uid}}/{{$table}}/{{$obj->id}}/2/产业资源/{{$obj->created_at}}','500','250')"><i class="icon-ok"></i> 退回通知</button>
				</div>
			@endif
		</div>
	</form>
        ...
        $("#form-admin").validate({
            ...
            submitHandler:function(form){
                ...
                $(form).ajaxSubmit({
                    ...
                    url: '/{{config('base.admin')}}/data/data_edit/{{$obj->id}}', // 需要提交的 url
