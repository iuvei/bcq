...
<article class="page-container">
	<form action="" method="post" class="form form-horizontal" id="form-admin">
		@foreach($fcategory as $val)
			@if($obj->id != $val->id && $obj->fid == $val->id)
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>父级分类(fid)：</label>
					<div class="formControls col-xs-8 col-sm-8">
				<span class="select-box">
					<select name="fid" class="select">
						@foreach($fcategory as $val)
							<option value="{{$val->id}}" @if($obj->fid == $val->id) selected @endif>{{$val->cname}}</option>
						@endforeach
					</select>
				</span>
					</div>
		...
		@endforeach
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>分类名称(cname)：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="{{$obj->cname}}" placeholder="" id="cname" name="cname">
			</div>
		</div>
		<div class="row cl">
		...
		@if($obj->recommend)
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">推荐(recommend)：</label>
	...
	$("#form-admin").validate({
		...
		submitHandler:function(form){
            ...
            $(form).ajaxSubmit({
                ...
                url: '/{{config('base.admin')}}/bcategory/category_edit/{{$obj->id}}', // 需要提交的 url
