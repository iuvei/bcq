...
<div class="page-container">
    <form method="POST" id="list_form" action="/{{config('base.admin')}}/official/official_list">
        {{ csrf_field() }}
        <div class="text-c">
            ...
            <button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜单页</button>
        </div>
    </form>
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a href="javascript:;" onclick="datastatus(1,'/{{config('base.admin')}}/official/datastatus')" class="btn btn-success radius"><i class="Hui-iconfont">&#xe6e1;</i> 批量启用</a>
            <a href="javascript:;" onclick="datastatus(0,'/{{config('base.admin')}}/official/datastatus')" class="btn btn-warning radius"><i class="Hui-iconfont">&#xe631;</i> 批量禁用</a>
            <a href="javascript:;" onclick="datastatus(-1,'/{{config('base.admin')}}/official/datastatus')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
            <a class="btn btn-primary radius" onclick="official_add('添加单页','/{{config('base.admin')}}/official/official_add','1200','')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加单页</a>
        </span>
...
<script type="text/javascript">
    $(function(){
        ...
        $('.table-sort').dataTable({
            ...
            "sAjaxSource": "/{{config('base.admin')}}/official/official_ajax_list",
            "aaSorting": [
                [0, "desc"],
            ],
            "aoColumns": [
                ...
                {"mData": "id","mRender": function ( data, type, full )
                {
                    return "<a title='编辑' href='javascript:;' onclick=\"official_edit('编辑','/{{config('base.admin')}}/official/official_edit/"+data+"','1200','')\" class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
                }
                }
            ],
            "oLanguage": {
    ...
    /*单页-添加*/
    function official_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
    /*单页-编辑*/
    function official_edit(title,url,w,h){
        layer_show(title,url,w,h);
    }
